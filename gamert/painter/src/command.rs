...
pub enum GpuRenderPassCommand {
    BindPipeline{pipeline: usize},
    BindShaderInput{pipeline_layout: usize, descriptor_sets: Vec<vk::DescriptorSet>},
    BindVertexBuffers{buffers: Vec<vk::Buffer>},
    BindIndexBuffer{buffer: vk::Buffer},
    SetPushConstant{pipeline_layout: usize, data: Vec<u8>},
    Draw{count: u32, vertex_offset: i32, index_offset: u32},
}
...
impl CommandBuffer {
    pub fn submit(&self, signal_semaphores: &[&GpuFuture], wait_semaphores: &[&GpuFuture], wait_stages: &[vk::PipelineStageFlags], fence: Option<&CpuFuture>) -> Result<(), String> {
        unsafe {
            let vk_fence = fence.map_or(vk::Fence::null(), |fence| fence.fence);
            let signal_semaphores = signal_semaphores.iter().map(|semaphore| semaphore.semaphore).collect::<Vec<_>>();
            let wait_semaphores = wait_semaphores.iter().map(|semaphore| semaphore.semaphore).collect::<Vec<_>>();
            self.painter.device.queue_submit(
                self.queue,
                &[vk::SubmitInfo::default().signal_semaphores(&signal_semaphores).wait_semaphores(&wait_semaphores).wait_dst_stage_mask(wait_stages).command_buffers(&[self.command_buffer])],
                vk_fence,
            )
    ...
    pub fn reset(&self) -> Result<(), String> {
        unsafe {
            self.painter.device.reset_command_buffer(self.command_buffer, vk::CommandBufferResetFlags::empty()).map_err(|e| format!("at command buffer reset: {e}"))?;
        }
        Ok(())
    }
    ...
    pub fn begin(&self, one_time: bool) -> Result<(), String> {
        unsafe {
            ...
            self.painter.device.begin_command_buffer(self.command_buffer, &vk::CommandBufferBeginInfo::default().flags(flags)).map_err(|e| format!("at command buffer begin: {e}"))?;
        }
        Ok(())
    }
    ...
    pub fn end(&self) -> Result<(), String> {
        unsafe {
            self.painter.device.end_command_buffer(self.command_buffer).map_err(|e| format!("at command buffer end: {e}"))?;
        }
        Ok(())
    }
    ...
    pub fn record(&mut self, commands: &[GpuCommand], one_time: bool) -> Result<(), String> {
        ...
        unsafe {
            self.painter.device.begin_command_buffer(self.command_buffer, &command_buffer_begin_info).map_err(|e| format!("at command buffer begin: {e}"))?;
            ...
            for (command_idx, command) in commands.iter().enumerate() {
                for transition in command.access_transitions() {
                    let (_, image_transitions) = image_accesses.entry(transition.image.image).or_insert((transition.image, vec![]));
            ...
            for (command_idx, command) in commands.iter().enumerate() {
                ...
                match command {
                    ...
                    GpuCommand::RunRenderPass { render_pass, framebuffer, extent, clear_values, pipelines, pipeline_layouts, commands: rp_commands } => {
                        ...
                        for rp_command in rp_commands.iter() {
                            match rp_command {
                                GpuRenderPassCommand::BindPipeline { pipeline } => {
                                    self.painter.device.cmd_bind_pipeline(self.command_buffer, vk::PipelineBindPoint::GRAPHICS, pipelines[*pipeline]);
                                },
                                GpuRenderPassCommand::BindShaderInput { pipeline_layout, descriptor_sets } => {
                                    self.painter.device.cmd_bind_descriptor_sets(self.command_buffer, vk::PipelineBindPoint::GRAPHICS, pipeline_layouts[*pipeline_layout], 0, descriptor_sets, &[]);
                                },
                                GpuRenderPassCommand::BindVertexBuffers { buffers } => {
                                    self.painter.device.cmd_bind_vertex_buffers(self.command_buffer, 0, buffers, &[0]);
                                },
                                GpuRenderPassCommand::BindIndexBuffer { buffer } => {
                                    self.painter.device.cmd_bind_index_buffer(self.command_buffer, *buffer, 0, vk::IndexType::UINT32);
                                },
                                GpuRenderPassCommand::SetPushConstant { pipeline_layout, data } => {
                                    self.painter.device.cmd_push_constants(self.command_buffer, pipeline_layouts[*pipeline_layout], vk::ShaderStageFlags::ALL, 0, data);
                                },
                                GpuRenderPassCommand::Draw { count, vertex_offset, index_offset } => {
                                    self.painter.device.cmd_draw_indexed(self.command_buffer, *count, 1, *index_offset, *vertex_offset, 0);
                                },
                            }
                        }
...
impl CommandPool {
    pub fn new(painter: Arc<Painter>) -> Result<Self, String> {
        unsafe {
            ...
            Ok(Self { command_pool, painter: painter.clone(), queue: painter.graphics_queue })
        }
    }
    ...
    pub fn allocate_command_buffers(&self, count: usize) -> Result<Vec<CommandBuffer>, String> {
        unsafe {
            let command_buffers = self
                ...
                .into_iter()
                .map(|command_buffer| CommandBuffer { command_buffer, command_pool: self.command_pool, queue: self.queue, painter: self.painter.clone() })
                .collect();
            Ok(command_buffers)
        }
    }
